...
This task is about generating the
most generalized form a qubit can take
from an initial state. By generalized,
this means that the qubit can be located at
any point on the surface of Bloch sphere by
changing the two parameters, theta and phi.
I interpreted the variational circuit as an
unitary matrix that transforms our initial state
to the generalized state, which is actually the U3
matrix. 
...
def check(final_state):
    ...
    if(1-(abs(final_state[0])+abs(final_state[1])) < 0.01):
        return True
